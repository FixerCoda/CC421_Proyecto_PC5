...
arquitectura lineal $\hat{V}(\sigma; w) = w^{\top}\varphi(\sigma)$ de la
...
\begin{align}
  y_t &= R_{t+1} + \gamma\, \hat{V}(\sigma_{t+1}; w), \label{eq:td-target}\\
  \delta_t &= y_t - \hat{V}(\sigma_t; w)
...
los episodios. El descuento $\gamma < 1$ (en la práctica $\gamma = 0.99$) actúa
como recurso algorítmico que estabiliza el \emph{bootstrapping}; el desempeño que
se reporta sigue siendo el total de líneas por partida.
...
para cada episodio:
    ...
    mientras no termine:
        ...
        delta = R + gamma * V(sigma') - V(sigma)       # V(terminal) = 0
